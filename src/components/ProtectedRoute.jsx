...
import { hasAnyRole, isActiveUser } from "../constants/roles.js";
...
export default function ProtectedRoute({ children, allowedRoles = [] }) {
      ...
      <div className="flex min-h-[55vh] items-center justify-center bg-slate-50 px-4">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 text-center shadow-sm">
          ...
          <p className="font-bold text-slate-700">
            {t("loading", { defaultValue: "Đang kiểm tra đăng nhập..." })}
          </p>
        </div>
      </div>
    );
  ...
  if (!isActiveUser(user) || (allowedRoles.length > 0 && !hasAnyRole(user, allowedRoles))) {
